...
def SaveModelToPath(model):
    ...
    StrNewModelPath = config("strPathOfModels") + config("strPathOfLitecoinModel")
...
def main():
    ...
    logging.info('check if the new model is better performing than the old Litecoin model')
    ...
    if blNewModelBetter:
        ...
        logging.info('Add the record with updated model = 1 to the model table Litecoin')
        SaveModelTrainDataToRealModelRunTable(strIdOftheCrypto=3, intModelRun=1, intModelUpdate=1, ftOldModelScore = ftOldModelScore, ftNewModelScore = ftNewModelScore, Conn=Conn)
    else:
        logging.info('Add the record with updated model = 0 to the model table Litecoin')
        SaveModelTrainDataToRealModelRunTable(strIdOftheCrypto=3, intModelRun=1, intModelUpdate=0, ftOldModelScore = ftOldModelScore, ftNewModelScore = ftNewModelScore, Conn=Conn)

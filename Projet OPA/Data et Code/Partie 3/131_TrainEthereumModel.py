...
def main():
    ...
    if blNewModelBetter:
        ...
        SaveModelTrainDataToRealModelRunTable(strIdOftheCrypto=98, intModelRun=1, intModelUpdate=1, Conn=Conn)
    else:
        logging.info('Add the record with updated model = 0 to the model Ethereum table')
        SaveModelTrainDataToRealModelRunTable(strIdOftheCrypto=98, intModelRun=1, intModelUpdate=0, Conn=Conn)

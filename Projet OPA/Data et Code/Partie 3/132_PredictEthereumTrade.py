...
def main():
    ...
    if prediction[-1] > dfCryptoData["price"].iloc[-1]:
        ...
        SaveModelPredictionDataToModelPredictionTable(strIdOftheCrypto=98,intBuySignal=0,intSellSignal=1, Conn=Conn)
    else:
        logging.info('Save Ethereum buy signal as the price is going to lower')
        SaveModelPredictionDataToModelPredictionTable(strIdOftheCrypto=98,intBuySignal=1,intSellSignal=0, Conn=Conn)

...
def main():
    ...
    logging.info('get the data from the database for Ethereum')
    dfCryptoData = Factory.ReadDataFromDatabase(engine, strCryptoQuery="strQueryEthereumTable")

    logging.info('Preparing data for prediction for Ethereum')
    X = PrepareData(dfCryptoData)

    logging.info('Loading prediction model for Ethereum')
    Prediction_Model = GetPredictionModel()

    logging.info('Perform prediction for Ethereum')
    ...
    if prediction[-1] > dfCryptoData["price"].iloc[-1]:
        logging.info('Save Ethereum sell signal as the price is going to rise')
        SaveModelPredictionDataToModelPredictionTable(strIdOftheCrypto=98,intBuySignal=0,intSellSignal=1, ftLastCryptoValue = dfCryptoData["price"].iloc[-1], ftPrediction = prediction[-1], Conn=Conn)
    else:
        logging.info('Save Ethereum buy signal as the price is going to lower')
        SaveModelPredictionDataToModelPredictionTable(strIdOftheCrypto=98,intBuySignal=1,intSellSignal=0, ftLastCryptoValue = dfCryptoData["price"].iloc[-1], ftPrediction = prediction[-1], Conn=Conn)
